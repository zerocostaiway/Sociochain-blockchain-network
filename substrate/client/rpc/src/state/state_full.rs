	...
	Client::Api: Metadata<Block>,
{
	...
	fn subscribe_runtime_version(&self, pending: PendingSubscriptionSink) {
			...
			.map_err(|e| Error::Client(Box::new(e)))
		{
			...
			Err(e) => {
				spawn_subscription_task(
					"rpc-subscription-state_subscribeRuntimeVersion_reject",
					&self.executor,
					pending.reject(e),
				);
				return
		...
		spawn_subscription_task(
			"rpc-subscription-state_subscribeRuntimeVersion",
			&self.executor,
			accept_and_pipe_from_stream(pending, stream, 16),
		);
	}
	...
	fn subscribe_storage(
		...
		if keys.is_none() {
			if let Err(err) = deny_unsafe.check_if_safe() {
				spawn_subscription_task(
					"rpc-subscription-state_subscribeStorage_reject",
					&self.executor,
					pending.reject(ErrorObject::from(err)),
				);
				return
			}
		}
		...
		spawn_subscription_task(
			"rpc-subscription-state_subscribeStorage",
			&self.executor,
			accept_and_pipe_from_stream(pending, stream, 16),
		);
	}
