	...
	B: BlockImport<Block, Error = ConsensusError> + Send + Sync,
{
	...
	pub fn set_sync_fork_request(
		...
		self.sync_service.set_sync_fork_request(peers.into_iter().map(
			|peer| peer.into()).collect()
		, hash, number);
	}
...
pub trait TestNetFactory: Default + Sized + Send {
	...
	fn add_full_peer_with_config(&mut self, config: FullPeerConfig) {
		...
		let peer_store = PeerStore::new(
			network_config.boot_nodes.iter().map(|bootnode| bootnode.peer_id.into()).collect(),
		);
		...
		self.mut_peers(move |peers| {
			for peer in peers.iter_mut() {
				peer.network
					.add_known_address(network.service().local_peer_id().into(), listen_addr.clone());
			}
